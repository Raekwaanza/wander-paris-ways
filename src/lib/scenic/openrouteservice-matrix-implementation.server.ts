...
import { validProviderPoint } from "./openrouteservice-routing-implementation.server";
...
export function validateWalkingMatrixInput(input: WalkingMatrixInput): WalkingMatrixInput {
  if (
    !input ||
    !Array.isArray(input.locations) ||
    input.locations.length < 2 ||
    input.locations.length > WALKING_MATRIX_LOCATION_LIMIT ||
    !input.locations.every(validProviderPoint)
  ) {
...
export async function walkingDurationMatrixImplementation(
  input: WalkingMatrixInput,
): Promise<WalkingMatrixResponse> {
  if (isScenicE2EFixtureMode()) {
    return { status: "success", durationsSeconds: fixtureWalkingDurations(input.locations) };
  }
  const apiKey = process.env["OPENROUTESERVICE_API_KEY"]?.trim();
  if (!apiKey) return { status: "unavailable" };
  const key = `${MATRIX_CACHE_VERSION}:${input.locations
  ...
  if (!request) {
    request = requestMatrix(input, apiKey);
