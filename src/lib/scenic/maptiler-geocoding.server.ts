...
function validPoint(input: unknown): input is LatLng {
...
function validateSearch(input: { query: string; proximity?: LatLng }) {
  ...
  return { query, proximity: validPoint(input.proximity) ? input.proximity : PARIS_SEARCH_CENTER };
}
...
export const searchParisWithMapTiler = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ForwardGeocodeResult> => {
    const params = new URLSearchParams({
      ...
      proximity: `${data.proximity.lng},${data.proximity.lat}`,
      types: MAPTILER_SEARCH_TYPES.join(","),
    });
    const payload = await mapTilerRequest(encodeURIComponent(data.query), params);
    return (
